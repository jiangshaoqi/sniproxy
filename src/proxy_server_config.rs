...
impl crypto::Session for ProxyTlsSession {
...
impl QuicProxyConfig {
    ...
    pub fn peek_server_name(&self, version: u32, params: &TransportParameters, client_hello: &[u8]) -> Option<String> {
        let version = interpret_version(version).unwrap();
        let mut server_name = None;
        let mut tls_session = ProxyTlsSession {
            version,
            got_handshake_data: false,
            next_secrets: None,
...
impl TryFrom<rustls::ServerConfig> for QuicProxyConfig {
    type Error = NoInitialCipherSuite;
...
impl TryFrom<Arc<rustls::ServerConfig>> for QuicProxyConfig {
    type Error = NoInitialCipherSuite;
...
impl crypto::ServerConfig for QuicProxyConfig {
    fn start_session(
        self: Arc<Self>,
        version: u32,
        params: &TransportParameters,
